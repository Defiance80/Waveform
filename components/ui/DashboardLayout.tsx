...
import { 
  ...
  Menu, X, User, Bell, ChevronLeft, LogOut
} from 'lucide-react';
    ...
    <div className="flex h-screen bg-[#0A0A0A] overflow-hidden">
      ...
      <div className={`hidden md:flex md:flex-col ${collapsed ? 'md:w-16' : 'md:w-64'} bg-[#0A0A0A] border-r border-[#1E1E1E] transition-all duration-300 flex-shrink-0`}>
        ...
        <div className="p-4 border-b border-[#1E1E1E]">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setCollapsed(!collapsed)} className="p-1.5 rounded-lg text-[#555] hover:text-white hover:bg-[#111] transition-all">
            ...
            return (
              <Link key={item.name} href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                  active 
                    ? 'text-white bg-[#111] border-l-2 border-[#00C2FF] shadow-[0_0_15px_rgba(0,194,255,0.1)]' 
                    : 'text-[#777] hover:text-white hover:bg-[#111]/50'
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="fixed left-0 top-0 bottom-0 w-72 bg-[#0A0A0A] border-r border-[#1E1E1E]">
            <div className="flex items-center justify-between p-4 border-b border-[#1E1E1E]">
              ...
              <button onClick={() => setSidebarOpen(false)} className="p-2 text-[#555] hover:text-white">
                <X size={20} />
              </button>
            </div>
                ...
                return (
                  <Link key={item.name} href={item.href} onClick={() => setSidebarOpen(false)}
                    className={`flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium transition-all ${
                      active ? 'text-white bg-[#111] border-l-2 border-[#00C2FF]' : 'text-[#777] hover:text-white hover:bg-[#111]/50'
                    }`}>
                    <item.icon size={18} className={active ? 'text-[#00C2FF]' : ''} />
                    <span>{item.name}</span>
                  </Link>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="bg-[#0A0A0A] border-b border-[#1E1E1E] px-4 py-3 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <button onClick={() => setSidebarOpen(true)} className="md:hidden p-2 text-[#777] hover:text-white rounded-lg">
                <Menu size={20} />
              </button>
              <div className="min-w-0">
                <h1 className="text-lg font-bold text-white truncate">Kendrick Cole</h1>
                <p className="text-xs text-[#555] hidden sm:block">Hip-Hop Artist • Atlanta, GA</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button className="relative p-2 text-[#555] hover:text-white transition-colors rounded-lg">
                <Bell size={18} />
                ...
                {userMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#111] border border-[#1E1E1E] rounded-xl shadow-xl z-50 overflow-hidden">
                    <div className="p-3 border-b border-[#1E1E1E]">
                      ...
                      <p className="text-xs text-[#555]">demo@waveform.ai</p>
                    </div>
                    <button onClick={handleLogout} className="w-full text-left px-3 py-2.5 text-sm text-[#777] hover:text-white hover:bg-[#1E1E1E] flex items-center gap-2 transition-colors">
        ...
        <footer className="hidden md:block border-t border-[#1E1E1E] px-4 py-2">
          <div className="flex justify-between text-xs text-[#333]">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0A0A0A]/95 backdrop-blur-lg border-t border-[#1E1E1E] px-1 py-1.5 z-40">
        <div className="flex items-center justify-around">
          {navigationItems.slice(0, 5).map((item) => {
            ...
            return (
              <Link key={item.name} href={item.href}
                className={`flex flex-col items-center py-1.5 px-3 rounded-lg transition-colors ${active ? 'text-[#00C2FF]' : 'text-[#555]'}`}>
                <item.icon size={18} />
